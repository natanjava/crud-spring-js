...
function criaLinha(dados){
    $("#dados").append(
        '<div class="linha" id="linha" name="linha">'+
            ...
            '<div class="coluna-4">'+
                '<button class="editar"  onclick="editar('+dados.codigo+')">Update</button>'+
                '<button class="excluir" onclick="deletar('+dados.codigo+')">Delete</button>'+
            '</div>'+
        '</div>'
    );
}
...
function ordenar(filtro){
    if(filtro == "escolha") gerarMessageBox(2, "Please select one option", "Try again");
    else{
        $.ajax({
            method: "GET",
            url: "/pessoa/"+filtro,
